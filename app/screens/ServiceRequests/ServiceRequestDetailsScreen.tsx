...
const ServiceRequestDetailsScreen = () => {
  ...
  const loadRequestDetails = useCallback(async () => {
    if (!requestId) {
      console.error('ServiceRequestDetailsScreen: No requestId provided');
      return;
    }
    if (hasFetchedRef.current) {
      console.log('ServiceRequestDetailsScreen: Already fetched, skipping');
      return;
    }
    ...
    try {
      ...
      // Check if we got an access denied response (limited data with message)
      if ((result as any).message && (result as any).message.includes('do not have access')) {
        console.error('ServiceRequestDetailsScreen: Access denied to request details');
      ...
      // Check if we got valid request data (should have key fields like seekerId, status, etc.)
      if (!result.seekerId && !(result as any).serviceLocation) {
        console.error('ServiceRequestDetailsScreen: Invalid request data received');
  ...
  useEffect(() => {
    ...
    if (!serviceRequest && requestId) {
      ...
      console.error('ServiceRequestDetailsScreen: No requestId or serviceRequest provided');
      Alert.alert('Error', 'Invalid request. Please try again.', [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);
    }
  }, [requestId, serviceRequest, loadRequestDetails, navigation]);
                ...
                [
                  {
                    text: 'View Order',
                    onPress: () =>
                      navigation.navigate('OrderDetail', { orderId: result.orderId }),
                  },
                ]
              );
            } catch (error: any) {
  ...
  const handleNavigateToOrder = () => {
    if (currentRequest?.orderId) {
      navigation.navigate('OrderDetail', { orderId: currentRequest.orderId });
    }
  };
  ...
  const isNoProvidersAvailable = requestStatus.toLowerCase() === 'no_providers_available';
  const canAccept =
    isProvider &&
    isSearching &&
    !isExpired;
  const canCancelRequest = isSeeker && !isExpired && !isNoProvidersAvailable;
  ...
  const getElapsedTime = () => {
    if (!isSearching || !currentRequest?.createdAt) return null;
  ...
  const formatHeaderSubtitle = () => {
    if (!currentRequest) return 'Service Request';
    const parts = [];
